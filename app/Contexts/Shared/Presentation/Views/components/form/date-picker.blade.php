@props([
    ...
    'name' => null,
    'dateFormat' => 'Y-m-d',
    'messages' => [], // Agregamos compatibilidad para errores como en tus otros componentes
])

<div x-data="{
    flatpickrInstance: null,
    {{-- Entrelazamos el modelo de Livewire directamente con Alpine --}}
    ...
    init() {
        this.$nextTick(() => {
            this.flatpickrInstance = flatpickr(this.$refs.dateInput, {
                ...
                {{-- Si hay un valor en Livewire (value), Flatpickr lo usará, si no, usa el defaultDate --}}
                defaultDate: this.value ? this.value : {{ $defaultDate ? (is_array($defaultDate) ? json_encode($defaultDate) : "'" . $defaultDate . "'") : 'null' }},
                onChange: (selectedDates, dateStr, instance) => {
                    {{-- Actualizamos el valor en Alpine/Livewire al cambiar la fecha --}}
                    this.value = dateStr;
            ...
            {{-- Observamos si Livewire cambia el valor externamente para actualizar Flatpickr --}}
            this.$watch('value', (newValue) => {
                if (this.flatpickrInstance && newValue !== this.flatpickrInstance.currentYear + '-' + this.flatpickrInstance.currentMonth) {
    ...
    <div class="relative custom-datepicker">
        <input
            ...
            {{-- Limpiamos el merge quitando las clases conflictivas por error --}}
        ...
        <div class="absolute inset-y-0 right-3 flex items-center pointer-events-none gap-2">
            {{-- Icono de error si existen mensajes --}}
            @if ($messages)
